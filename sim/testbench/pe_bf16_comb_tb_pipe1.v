...
//  7. MUL: large * large â inf clamp
//  8. MAC: negative accumulator: -4.0 + 2.0*3.0 = 2.0

`timescale 1ns/1ps

module pe_bf16_comb_tb_pipe1;
    ...
    initial begin
        ...
        // T7: MAX * MAX â should clamp to INF or large
        begin
            op_mac = 1'b0; A = BF16_MAX; B = BF16_MAX; C = 16'h0;
            #10;
            $write("Test 7 [MAX*MAX~INF]: Y=%h  ", Y);
            // exponent will overflow â INF = 7F80
            if (Y[14:7] == 8'hFF) begin
                $display("PASS (overflowâinf/nan exp=FF)");
                pass_cnt = pass_cnt + 1;
            end else begin
        ...
        // T13: MAC C=0, A=0.5, B=0.5 â 0.25
