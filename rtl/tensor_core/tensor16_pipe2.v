...
reg [6:0]  mantissa_mul_output_stage3, mantissa_mul_output_stage4;
reg [7:0]  exponent_mul_output_stage2, exponent_mul_output_stage3, exponent_mul_output_stage4;
reg [15:0] fb16_C_stage2, fb16_C_stage3, fb16_C_stage4 ;
reg sign_mul_output_stage2, sign_mul_output_stage3, sign_mul_output_stage4;
reg op_mac_stage2, op_mac_stage3, op_mac_stage4;

	 	always @(posedge clk) begin
        if (reset || pc_reset) begin
				fb16_C_stage2 <= 8'b0;
				op_mac_stage2 <= 1'b0;
				exponent_mul_output_stage2 <= 8'b0;
	 	...
	 	always @(posedge clk) begin
        if (reset || pc_reset) begin
		      mantissa_mul_output_stage3 <= 8'b0;
				fb16_C_stage3 <= 8'b0;
				op_mac_stage3 <= 1'b0;
				exponent_mul_output_stage3 <= 8'b0;
				sign_mul_output_stage3 <= 1'b0;
        end else begin
		      mantissa_mul_output_stage3 <= mantissa_mul_output;
				fb16_C_stage3 <= fb16_C_stage2;
				op_mac_stage3 <= op_mac_stage2;
				exponent_mul_output_stage3 <= exponent_mul_output_stage2;
				sign_mul_output_stage3 <= sign_mul_output_stage2;
        end
    end
/*
	always @(posedge clk) begin
        if (reset || pc_reset) begin
            mantissa_mul_output_stage4 <= 8'd0;
				exponent_mul_output_stage4 <= 7'b0;
				sign_mul_output_stage4 <= 1'b0;
				fb16_C_stage4 <= 8'b0;
				op_mac_stage4 <= 1'b0;
        end else begin
            mantissa_mul_output_stage4 <= mantissa_mul_output;
				exponent_mul_output_stage4 <= exponent_mul_output_stage3;
				sign_mul_output_stage4 <= sign_mul_output_stage3;
				fb16_C_stage4 <= fb16_C_stage3;
				op_mac_stage4 <= op_mac_stage3;
        end
    end
*/
//mac logic  num_1 = A*B, num_2 = C
wire sign_C = fb16_C_stage3[15];
wire [7:0] exponent_C = fb16_C_stage3[14:7];
wire [6:0] mantissa_C = fb16_C_stage3[6:0];
//intermid calculation
wire [7:0] mantissa_mac_cal1 = {1'b1, mantissa_mul_output_stage3}; // add
wire [7:0] mantissa_mac_cal2 = {1'b1, mantissa_C}; // add implicit leading 1 for C

wire sign_mac_cal = sign_mul_output_stage3 ^ sign_C; // XOR for sign of the result
wire exp_cmp = exponent_C > exponent_mul_output_stage3; // Compare exponents to determine which is larger
wire exp_equal = exponent_C == exponent_mul_output_stage3; // Check if exponents are equal
...
wire [8:0] shift_mac_exponent = shift_signal ? (exponent_C - exponent_mul_output_stage3) : (exponent_mul_output_stage3 - exponent_C); // Absolute value of exponent difference
...
assign sign_mac_output = sign_mac_cal ? ( shift_signal ? sign_C : sign_mul_output_stage3) : sign_C ; 
...
assign exponent_mac_output = (shift_signal ? exponent_C : exponent_mul_output_stage3) + carry_mac - exponent_shift_offset; // Use the larger exponent
...
assign result = op_mac_stage3 ? mac_result : mul_result; // Select between MAC and MUL results based on control signal
