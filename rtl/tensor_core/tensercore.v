...
wire sign_C = fb16_C[15];
wire [7:0] exponent_C = fb16_C[14:7];
wire [6:0] mantissa_C = fb16_C[6:0];
//intermid calculation
wire [7:0] mantissa_mac_cal1 = {1'b1, mantissa_mul_output}; // add
wire [7:0] mantissa_mac_cal2 = {1'b1, mantissa_C}; // add implicit leading 1 for C

wire sign_mac_cal = sign_mul_output ^ sign_C; // XOR for sign of the result
wire exp_cmp = exponent_C > exponent_mul_output; // Compare exponents to determine which is larger
wire exp_equal = exponent_C == exponent_mul_output; // Check if exponents are equal
...
wire [8:0] shift_mac_exponent = shift_signal ? (exponent_C - exponent_mul_output) : (exponent_mul_output - exponent_C); // Absolute value of exponent difference
...
assign sign_mac_output = sign_mac_cal ? ( shift_signal ? sign_C : sign_mul_output) : sign_C ; 
...
assign result = op_mac ? mac_result : mul_result; // Select between MAC and MUL results based on control signal
